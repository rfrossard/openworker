...
Original visual.`);
  ...
  it("applies template tokens to the preview and offers twenty-eight slide styles", async () => {
    ...
    expect(screen.getByLabelText("Slide styles").querySelectorAll("button")).toHaveLength(37);
    expect(screen.getByLabelText("Slide styles").textContent).toContain("Map");
...
\`\`\``,
  ...
  it("prefills an editable visual recommendation from the slide and selected template", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("slide-preview")).toBeTruthy());
    fireEvent.click(screen.getByRole("button", { name: "Continue to design" }));
    expect(screen.getByText("Visual mood (for generated images)")).toBeTruthy();
    expect((screen.getByRole("option", { name: "Match template" }) as HTMLOptionElement).selected).toBe(true);
    expect(screen.getByRole("option", { name: "Human & editorial" })).toBeTruthy();
